...
export default defineConfig(({ mode }) => {
  ...
  // CRIMENEXA_API.pdf: "CORS is currently configured for localhost:3000 and
  // localhost:8080 on both services." The backend/AI service code isn't
  // being touched, so the dev server must run on one of those two ports —
  // 8080 was picked since it doesn't collide with anything else in this
  // project. Override with `PORT=3000` (or any other allowed port) if you
  // add it to CorsConfig.java / CORSMiddleware later.
  const devPort = parseInt(process.env.PORT || '3000')
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ['**/.figma/**'] },
      // Neither backing service allows this dev server's origin. Their CORS
      // allowlists cover http://localhost:3000 (both) and http://localhost:5173
      // (backend only), but this server runs on $PORT / 8443, so every browser
      // request is rejected before it reaches the app.
      //
      // Proxying sidesteps CORS entirely: the browser talks same-origin to Vite,
      // and Vite makes the upstream call server-side where CORS does not apply.
      // src/lib/config.ts points at these prefixes in dev.
      //
      // The Origin/Referer headers must also be STRIPPED, not just forwarded.
      // http-proxy passes the browser's `Origin: http://localhost:8443` upstream,
      // and Spring Security's CORS filter actively rejects an unlisted origin
      // with `403 Invalid CORS request` - even server-to-server. (Starlette is
      // laxer: it just omits the response headers, which is why the AI service
      // appeared to work without this.) A genuine server-to-server call has no
      // browser origin, so removing the header is the honest representation.
      proxy: {
        '/ai-api': {
          target: process.env.VITE_AI_SERVICE_URL || 'https://crimenexa-ai-service.onrender.com',
          changeOrigin: true,
          rewrite: (p) => p.replace(/^\/ai-api/, ''),
          configure: stripBrowserOrigin,
        },
        '/be-api': {
          target: process.env.VITE_BACKEND_URL || 'https://crimenexa-backend.onrender.com',
          changeOrigin: true,
          rewrite: (p) => p.replace(/^\/be-api/, ''),
          configure: stripBrowserOrigin,
        },
      },
    },
...
/**
 * Removes the browser's Origin/Referer from proxied requests so the upstream
 * service treats them as ordinary server-to-server calls rather than
 * cross-origin browser traffic. Without this the backend answers
 * `403 Invalid CORS request`.
 */
// Typed loosely: Vite re-exports http-proxy's ProxyServer, whose types are not
// worth importing for two removeHeader calls.
